...
function useAuth() {
  ...
  const requestAuth = useCallback(() => {
    ...
    console.log(
      "[useAuth] Token value (first 30 chars):",
      token.slice(0, 30) + "...",
    );
    console.log("[useAuth] Token length:", token.length);
    console.log(
      "[useAuth] Token format check:",
      token.startsWith("Bearer ") ? "Has Bearer prefix" : "No Bearer prefix",
    );
    ...
    if (!tokenBeforeRequest || tokenBeforeRequest.trim() === "") {
      console.error(
        "[useAuth] ERROR: Token disappeared from localStorage before request!",
      );
      setState(1);
    ...
    console.log(
      "[useAuth] ===== Making GET request to /api/v1/auth/user =====",
    );
    console.log(
      "[useAuth] APIBase.defaults.baseURL:",
      APIBase.defaults?.baseURL,
    );
    console.log(
      "[useAuth] Full URL will be:",
      `${APIBase.defaults?.baseURL || ""}/api/v1/auth/user`,
    );
    console.log("[useAuth] Token will be added by ApiBase interceptor");
    console.log(
      "[useAuth] Token in localStorage:",
      tokenBeforeRequest ? "EXISTS" : "MISSING",
    );
    if (tokenBeforeRequest) {
      console.log(
        "[useAuth] Token (first 30 chars):",
        tokenBeforeRequest.slice(0, 30) + "...",
      );
      console.log("[useAuth] Token length:", tokenBeforeRequest.length);
      console.log(
        "[useAuth] Token format:",
        tokenBeforeRequest.startsWith("Bearer ")
          ? "Has Bearer prefix (will be cleaned)"
          : "No Bearer prefix (OK)",
      );
    }
    ...
    if (!finalTokenCheck || finalTokenCheck.trim() === "") {
      console.error(
        "[useAuth] ❌❌❌ CRITICAL: Token disappeared right before request!",
      );
      setState(1);
      requestAuthInProgressRef.current = false;
      return Promise.reject(new Error("Token not found in localStorage"));
    ...
    return APIBase.get("/api/v1/auth/user")
      .then((payload) => {
        console.log(
          "[useAuth] ✓ API response received, status:",
          payload.status,
        );
        return payload.data;
      })
      .then((data) => {
        ...
        if (data) {
          ...
          if (
            data.account &&
            data.account.roles &&
            Array.isArray(data.account.roles)
          ) {
            ...
            console.log(
              "[useAuth] ✓ Roles normalized and stored as objects:",
              normalizedRoleNames,
            );
            console.log("[useAuth] Final roles structure:", data.account.roles);
          } else {
            console.warn(
              "[useAuth] ⚠ No roles in user data or roles is not an array",
            );
          ...
          return new Promise((resolve) => {
            requestAnimationFrame(() => {
              requestAnimationFrame(() => {
                ...
                console.log(
                  "[useAuth] ✓ State set to loaded (1), Redux should have user",
                );
                resolve(data);
              });
            });
      ...
      .catch((e) => {
        ...
        console.error(
          "[AUTH ERROR] Has Authorization Header:",
          !!e?.config?.headers?.Authorization,
        );
        console.error(
          "[AUTH ERROR] Authorization Header Value:",
          e?.config?.headers?.Authorization
            ? e.config.headers.Authorization.slice(0, 30) + "..."
            : "MISSING",
        );
        console.error(
          "[AUTH ERROR] Token in localStorage:",
          window.localStorage.getItem("AUTH_TOKEN") ? "EXISTS" : "MISSING",
        );
        console.error("[AUTH ERROR] ========================================");
        ...
        if (status === 403) {
          ...
          console.error(
            "[AUTH ERROR] 2. User doesn't have permission to access /api/v1/auth/user",
          );
          console.error(
            "[AUTH ERROR] 3. JWT filter didn't set authentication correctly",
          );
          console.error(
            "[AUTH ERROR] 4. Token format is incorrect (missing Bearer prefix?)",
          );
          ...
          console.error(
            "[AUTH ERROR] NOTE: This error is non-blocking if called after login.",
          );
          console.error(
            "[AUTH ERROR] Login flow will continue with user data from login response.",
          );
          console.error(
            "[AUTH ERROR] ========================================",
          );
          ...
          console.error(
            "[AUTH ERROR] 401 Unauthorized - Token is invalid or missing",
          );
        }
  ...
  useEffect(() => {
    ...
    if (user && user.id) {
      console.log(
        "[useAuth] ✓✓✓ User already in Redux on mount (from login), skipping requestAuth()",
      );
      console.log("[useAuth] User ID:", user.id);
      console.log(
        "[useAuth] User roles:",
        user?.account?.roles?.map((r) => r?.name || r) || [],
      );
      console.log(
        "[useAuth] Setting state to loaded - user data is already available",
      );
      setState(1);
      hasRequestedRef.current = true;
      return;
    ...
    if (token && token.trim() !== "") {
      ...
      console.log(
        "[useAuth] ⚠ Token exists but no user in Redux - calling requestAuth()",
      );
      console.log(
        "[useAuth] This might happen on page refresh or if Redux state was lost",
      );
      hasRequestedRef.current = true;
      requestAuth().catch((error) => {
        ...
        if (status === 401 && user) {
          console.warn(
            "[useAuth] 401 error but user exists in Redux - keeping user data",
          );
          console.warn(
            "[useAuth] This is likely a token format issue, not an auth failure",
          );
  ...
  const role = useMemo(() => {
    ...
    if (
      !user.account ||
      !user.account.roles ||
      !Array.isArray(user.account.roles)
    ) {
      return ["GUEST"];
    }
