...
function UserCart() {
  ...
  const scrollToLoad = useCallback(() => {
    ...
    if (scrollTop + windowHeight >= docHeight - 100) {
      fetchCart(pageRef.current.index);
    }
  }, [fetchCart, user]);
  ...
  }, [user, fetchCart, scrollToLoad]);

  useEffect(() => {
    setSelectedItem((selected) =>
      selected.filter((selectedItem) =>
        data.some((item) => item.id === selectedItem.id),
      ),
    );
  }, [data]);
  ...
  function onCheckItem(e) {
    if (e.target.checked) {
      setSelectedItem((selectedItem_) => {
        var temp = data.find(
          (item_) => item_.id === Number.parseInt(e.target.value),
        );
  ...
  function handleItemChange(id, cartItem) {
    APIBase.put(`/api/v1/cart/${id}`, cartItem)
      .then((payload) => {
        setData((item_) => {
          for (var i = 0; i < item_.length; i++) {
              ...
              description={
                <div>
                  ...
                  <p style={{ color: "#8c8c8c", marginBottom: 16 }}>
                    Already placed an order? Track your order status in{" "}
  ...
  return (
    <Row gutter={[12, 12]} className={style.cartLayout}>
            ...
            return (
              ...
              >
                <Col xs={2} sm={2} md={2} lg={1}>
                  <input
                    ...
                    checked={selectedItems.some(
                      (selectedItem_) => selectedItem_.id === item.id,
                    )}
                    value={item.id}
                ...
                <Col xs={20} sm={20} md={20} lg={22}>
                  <OrderItem
                    onChange={(payload) => handleItemChange(item.id, payload)}
                    data={item}
                  />
                  <div
                    style={{
                  ...
                  >
                    ...
                    {isInvalid && (
                      <span style={{ color: "#ff4d4f", fontSize: "12px" }}>
                        <ExclamationCircleOutlined /> Please reduce quantity or
                        remove item
                      </span>
                    )}
                  </div>
      ...
      <Col span={24} md={{ span: 10 }} lg={{ span: 9 }}>
        <Card title="Summary" className={style.summaryCard}>
          <h4 className={style.summaryValue}>
            <Currency
              value={selectedItems.reduce((pre, item) => {
          ...
          <Button
            ...
            block
            disabled={
              selectedItems.length === 0 || hasOutOfStock(selectedItems)
            }
            onClick={() => {
              navigate("/checkout", {
                state: {
