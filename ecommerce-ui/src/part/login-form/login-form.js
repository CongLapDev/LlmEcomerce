...
import { LOCAL_URL } from "../../api/ApiBase";
  ...
  const handleLoginSubmit = async (values) => {
    ...
    try {
      ...
      console.log(
        ...
        JSON.stringify({ username: values.username, password: "***" }),
      );
      ...
      if (!savedToken || savedToken.trim() === "") {
        ...
        console.error(
          "[LOGIN ERROR] Authentication token not received. Please try again.",
        );
      ...
      if (userData && userData.account) {
        if (userData.account.roles && Array.isArray(userData.account.roles)) {
          const normalizedRoleNames = userData.account.roles
            ...
            .filter(Boolean);
          userData.account.roles = normalizedRoleNames.map((name) => ({
            name,
          }));
          ...
          console.warn(
            "[LOGIN] ⚠ No roles in login response or roles is not an array",
          );
          console.warn(
            "[LOGIN] userData.account.roles:",
        ...
        console.error(
          "[LOGIN] ERROR: userData or userData.account is missing!",
        );
      ...
      console.log(
        "[LOGIN] ✓ Redux state updated with user data from login response",
      );
      console.log("[LOGIN] User ID in Redux:", userData?.id);
      console.log(
        "[LOGIN] User roles in Redux:",
        userData?.account?.roles?.map((r) => r.name) || [],
      );
      ...
      console.log(
        "[LOGIN] Waiting for Redux state propagation and useAuth sync...",
      );
      await new Promise((resolve) => {
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              console.log(
                "[LOGIN] ✓ Redux state propagation and useAuth sync complete",
              );
              console.log(
                "[LOGIN] useAuth should now see user in Redux and state should be 1",
              );
              resolve();
            });
          });
      ...
      console.log(
        "[LOGIN] Step 3.7: Skipping requestAuth() - we already have user data from login",
      );
      console.log(
        "[LOGIN] User data is already in Redux, state is set to loaded",
      );
      console.log(
        "[LOGIN] requestAuth() will be called automatically on next page load if needed",
      );
      ...
      // ============================================
      const userRolesRaw =
        userData?.account?.roles?.map((r) => r.name || r) || [];
      ...
      if (isAdmin) {
        ...
        globalContext.message.warning(
          "Login successful but no role assigned. Please contact administrator.",
        );
      ...
      if (targetRoute) {
        ...
        setTimeout(() => {
          ...
          if (currentPath === targetRoute) {
            ...
            console.log(
              "[LOGIN] User is now authenticated and on",
              targetRoute,
            );
            console.log(
              "[LOGIN] Home page should load with authenticated state",
            );
            isNavigatingRef.current = false;
            ...
            console.error(
              "[LOGIN] Attempting fallback navigation via window.location.href...",
            );
            ...
            console.warn(
              "[LOGIN] ⚠ Navigation may be in progress or redirected elsewhere",
            );
            console.warn(
              "[LOGIN] Current path:",
              currentPath,
        ...
        setTimeout(() => {
          const currentPath = window.location.pathname;
          console.log(
            "[LOGIN] ===== POST-NAVIGATION VERIFICATION (800ms) =====",
          );
          ...
          if (currentPath === targetRoute) {
            ...
            console.error(
              "[LOGIN] ✗✗✗ NAVIGATION STILL FAILED - Using window.location.assign()",
            );
            console.error("[LOGIN] React Router navigation completely failed");
            console.error(
              "[LOGIN] Forcing navigation with window.location.assign()...",
            );
            window.location.assign(targetRoute);
          } else {
            console.warn("[LOGIN] ⚠ User is on different route:", currentPath);
            console.warn(
              "[LOGIN] This might be intentional (e.g., redirect by RoleBaseAuthorize)",
            );
          }
          isNavigatingRef.current = false;
        }, 800);
        setTimeout(() => {
          ...
          if (finalPath !== targetRoute) {
            ...
            console.warn(
              "[LOGIN] This might indicate a routing issue or RoleBaseAuthorize redirect",
            );
          }
        }, 800);
      }
      ...
      if (error?.response) {
        ...
        console.error(
          "[LOGIN ERROR] - Request Base URL:",
          error.config?.baseURL,
        );
        console.error(
          "[LOGIN ERROR] - Full URL:",
          error.config?.baseURL + error.config?.url,
        );
        console.error(
          "[LOGIN ERROR] - Request Headers:",
      ...
      if (error?.response) {
        ...
        if (status === 401) {
          ...
          errorMessage =
            "Access denied. Your account may not have permission to login.";
        } else if (status === 404) {
          errorMessage = "Login endpoint not found. Please contact support.";
          console.error(
            "[LOGIN ERROR] Endpoint /api/auth/login returned 404 - backend might not have this endpoint",
          );
        } else if (status === 400) {
          errorMessage =
      ...
      console.error(
        "[LOGIN ERROR] Displaying error message to user:",
        errorMessage,
      );
    ...
    <div className={clsx("w-100", className)}>
      <form noValidate onSubmit={formik.handleSubmit}>
        <Row gutter={[0, 24]} justify={"center"}>
          ...
          <Col span={24}>
            <Row justify={isAdminLogin ? "center" : "end"} gutter={[8, 8]}>
              <Col
                className={style.loginBtn}
                span={isAdminLogin ? 24 : undefined}
              >
                <Button
                  htmlType="submit"
                  type="primary"
          ...
          {!isAdminLogin && (
            <>
              ...
              <Col span={24}>
                <Description>Or login with</Description>
              </Col>
              <Col span={16}>
                <Row>
                  <Col span={6}>
                    <Button
                      ...
                      onClick={() => {
                        const backendUrl = (LOCAL_URL || "").replace(/\/$/, "");
