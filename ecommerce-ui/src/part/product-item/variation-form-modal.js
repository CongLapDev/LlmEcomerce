...
function VariationOptionSelect({
  name,
  variations,
  setVariations,
  remove,
      ...
      .then((data) => {
        ...
        setVariations((variations) => {
          for (var i = 0; i < variations.length; i++) {
            if (variations[i].id === data.variation.id)
              variations[i].options = [
                data,
                ...(Array.isArray(variations[i].options)
                  ? variations[i].options
                  : []),
              ];
            break;
      ...
      <Col span={10}>
        ...
        >
          <Select
            options={
              variations &&
              variations.map((variation_) => ({
                label: variation_.name,
                value: variation_.id,
              }))
            }
            ...
            onChange={(value) => {
              setSelectedVariation(
                variations.find((variation_) => variation_.id === value),
              );
              ...
              <>
                ...
                <Form onFinish={addVariation}>
                  <Space.Compact style={{ width: "100%" }}>
                    <Form.Item style={{ padding: 0 }} name="name">
                      <Input name="name" placeholder="Variation" />
                    </Form.Item>
        ...
        >
          <Select
            options={
              ...
              selectedVariation.options.map((option_) => ({
                label: option_.value,
                value: option_.id,
              }))
            }
            dropdownRender={(menu) => (
              <>
                ...
                <Form onFinish={addOption}>
                  <Space.Compact style={{ width: "100%" }}>
                    <Form.Item style={{ padding: 0 }} name="value">
...
function VariationForm({ submitHandler, loading, onCancel, product }) {
  ...
  useEffect(() => {
    APIBase.get(`/api/v1/variation`).then((payload) => {
      setVariations(payload.data);
    });
  }, []);
    ...
    var productItem = {
      ...
      options: data.options
        .map((option_) => {
          ...
          const variationId =
            typeof option_.variation === "object"
  ...
  return (
    <Form onFinish={buildFormData}>
      <Row gutter={14}>
          ...
          >
            ...
            >
              <Button
                icon={
                  <PrefixIcon>
                    <i className="fi fi-rr-inbox-out"></i>
                  </PrefixIcon>
                }
              >
                Click to Upload
        ...
        <Col span={24}>
          <Flex style={{ paddingTop: "24px" }} justify="end">
            <Button htmlType="submit" type="primary">
              Submit
            </Button>
          </Flex>
