...
function ProductItemView({ productItem, setData }) {
  ...
  const options = Array.isArray(productItem?.options)
    ? productItem.options
    : [];
  ...
  function deleteItem(id) {
    APIBase.delete(`/api/v1/product/0/item/${id}`)
      .then(() => {
        setData((product) => {
  ...
  return (
    <>
      <tr>
        <td rowSpan={rowSpan}>{productItem.id}</td>
        <td rowSpan={rowSpan}>
          <Image width="100px" src={getImageUrl(productItem.picture)} />
        </td>
        <td>{firstOption.variation?.name || "N/A"}</td>
        <td>{firstOption.value || "N/A"}</td>
        <td rowSpan={rowSpan}>{productItem.originalPrice || 0}</td>
        <td onDoubleClick={() => setEditable(true)} rowSpan={rowSpan}>
          <Space>
            ...
            <Button
              ...
              icon={
                <PrefixIcon>
                  <i style={{ color: "white" }} className="fi fi-br-check"></i>
                </PrefixIcon>
              }
            />
          </Space>
        ...
        <td rowSpan={rowSpan}>
          <Button type="text" onClick={() => deleteItem(productItem.id)}>
            Delete
          </Button>
        </td>
      </tr>
